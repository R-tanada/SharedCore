...
class RPLidar {
    ...
    rp_values::ComResult send_packet(rp_values::OrderByte order,
                                     const std::vector<uint8_t> &payload = {});
    rp_values::ComResult read_scan_data(std::vector<uint8_t> &output_data,
                                        uint8_t size, bool to_sync = false);
   ...
   public:
    ...
    rp_values::ComResult get_samplerate(
        data_wrappers::SampleRateData *sample_rate);
