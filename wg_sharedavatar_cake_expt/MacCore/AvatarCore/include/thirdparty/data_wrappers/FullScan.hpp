...
struct FullScan {
    ...
    ExpressScanPacket
        next_packet;  // Next express packet data(formula in com. protocol
                      // datasheet requires two consecutive scans, cf p23)
    uint16_t current_id = 0;
    uint8_t measurement_id =
        CFG_NB_SAMPLE_PER_PACKET;  // To go through the 32 measurements in each
                                   // express packet
    ...
    void clear() {
        std::fill(measurements.begin(), measurements.end(),
                  std::pair<float, uint16_t>(0, 0));
    }

    ssize_t size() { return measurements.size(); }
    ...
    /**
     * Gets the "measurement_id"th measurement in the current packet, needs the
     * next packet also
     ...
     * @return Measurement struct (includes distance and angle of the
     * measurement)
     */
    std::pair<float, uint16_t> get_next_measurement(
        const ExpressScanPacket& scan_packet, float next_angle,
        uint8_t measurement_id) {
        uint16_t distance = scan_packet.distances[measurement_id];

        // Formulas for angle from slamtec protocol manual
        float angle_diff = (next_angle >= scan_packet.start_angle)
                               ? (next_angle - scan_packet.start_angle)
                               : (360 + next_angle - scan_packet.start_angle);
        float angle = fmodf(scan_packet.start_angle +
                                (angle_diff / 32.0f) * measurement_id -
                                scan_packet.d_angles[measurement_id - 1],
                            360.0f);
    ...
    bool compute_measurements() {
        for (measurement_id = 0; measurement_id < CFG_NB_SAMPLE_PER_PACKET;
             measurement_id++) {
            std::pair<float, uint16_t> m = get_next_measurement(
                current_packet, next_packet.start_angle, measurement_id);
            add_measurement(m);
        }
        return false;
    }
