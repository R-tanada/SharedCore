...
class line3d {
   public:
    ...
    line3d(float sX, float sY, float sZ, float eX, float eY, float eZ)
        : pStart(sX, sY, sZ), pEnd(eX, eY, eZ){};

    line3d &operator+=(const Point3df &p) {
        pStart += p;
        pEnd += p;
        return *this;
    }
    ...
    line3d operator+(const Point3df &p) const {
        return line3d(pStart + p, pEnd + p);
    }
    line3d operator-(const Point3df &p) const {
        return line3d(pStart - p, pEnd - p);
    }
    ...
    bool operator==(const line3d &l) const {
        return pStart == l.pStart && pEnd == l.pEnd;
    }
    bool operator!=(const line3d &l) const {
        return pStart != l.pStart || pEnd != l.pEnd;
    }

    double Length() const { return pStart.getDist(pEnd); }
    double LengthSQ() const { return pStart.getDistSQ(pEnd); }

    Point3df getVector() const { return (pEnd - pStart); }
    ...
    Point3df UnitVector() const {
        return (Point3df)(pEnd - pStart).Normalize();
    }
    Point3df getMiddle() const { return (pStart + pEnd) * 0.5f; }
