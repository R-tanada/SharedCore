...
class Point2d {
   public:
    ...
    bool Equal(const Point2d<T> &p) { return fEqual(x, p.x) && fEqual(y, p.y); }

    double LengthSQ() { return x * x + y * y; };
    double Length() { return sqrt(LengthSQ()); };

    T dotProduct(const Point2d<T> &p) { return x * p.x + y * p.y; }

    double getDistSQ(const Point2d<T> &p) {
        return Point2d<T>(x - p.x, y - p.y).LengthSQ();
    }
    double getDist(const Point2d<T> &p) { return sqrt(getDistSQ(p)); }
...
Point2d<T> operator*(const S s, Point2d<T> &p) {
    return p * s;
}
