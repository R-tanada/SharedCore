...
class rect {
   public:
    ...
    rect<T> operator+(const Point2d<T> &p) const {
        return rect<T>(ULPoint + p, BRPoint + p);
    };
    rect<T> operator-(const Point2d<T> &p) const {
        return rect<T>(ULPoint - p, BRPoint - p);
    };
    rect<T> &operator+=(const Point2d<T> &p) {
        ULPoint += p;
        BRPoint += p;
        return *this;
    };
    ...
    bool operator==(const rect<T> &r) const {
        return (ULPoint == r.ULPoint && BRPoint == r.BRPoint);
    };
    bool operator!=(const rect<T> &r) const {
        return (ULPoint != r.ULPoint || BRPoint != r.BRPoint);
    };
    ...
    bool IsPointInside(const Point2d<T> &p) const {
        return (p.x > ULPoint.x && p.x < BRPoint.x && p.y > ULPoint.y &&
                p.y < BRPoint.y);
    }
    ...
    void Repair() {
        if (ULPoint.x > BRPoint.x) Math::Swap<T>(ULPoint.x, BRPoint.x);
        if (ULPoint.y > BRPoint.y) Math::Swap<T>(ULPoint.y, BRPoint.y);
    }

    void clipRect(const rect<T> &rc) {
        if (ULPoint.y < rc.ULPoint.y) ULPoint.y = rc.ULPoint.y;
        if (ULPoint.x < rc.ULPoint.x) ULPoint.x = rc.ULPoint.x;
        if (BRPoint.y > rc.BRPoint.y) BRPoint.y = rc.BRPoint.y;
        if (BRPoint.x > rc.BRPoint.x) BRPoint.x = rc.BRPoint.x;
        if (ULPoint.y > BRPoint.y) ULPoint.y = BRPoint.y;
        if (ULPoint.x > BRPoint.x) ULPoint.x = BRPoint.x;
    }
    Point2d<T> getCenter() const {
        return Point2d<T>((BRPoint.x + ULPoint.x) / 2,
                          (BRPoint.y + ULPoint.y) / 2);
    }

    T getHeight() const { return BRPoint.y - ULPoint.y; }
    T getWidth() const { return BRPoint.x - ULPoint.x; }

    Point2d<T> getSize() const { return Point2d<T>(getWidth(), getHeight()); }

    bool isValid() const { return (getWidth() > 0 && getHeight() > 0); }

    void reset(const Point2d<T> &p) { BRPoint = ULPoint = p; }
