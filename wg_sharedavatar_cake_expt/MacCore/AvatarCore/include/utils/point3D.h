...
class Point3d {
   public:
    ...
    inline Point3d<T> crossProduct(const Point3d<T> &p) const {
        return Point3d<T>(y * p.z - z * p.y, z * p.x - x * p.z,
                          x * p.y - y * p.x);
    }
    ...
    Point3d<T> TranslateByAngles(const Point3d<T> &abs,
                                 const Point3d<T> &angles) {
        Point3d<T> out = *this;
    ...
    Point3d<T> rotateX(double Deg,
                       const Point3d<T> &center = Point3d<T>(0, 0, 0)) {
        Deg = Math::toRad(Deg);
        float cs = cos(Deg);
        float sn = sin(Deg);
        y -= center.y;
    ...
    Point3d<T> getVecAngle(const Point3d<T> &vecTo) const {
        ...
        if (cs != 0) {
            float ang = vec.x / cs;
            if (abs(ang) > 1) ang = ang > 0 ? 1 : -1;
            angles.y = -asin(ang);
            if (vec.z > 0) angles.y = Math::PI64 - angles.y;
        }
