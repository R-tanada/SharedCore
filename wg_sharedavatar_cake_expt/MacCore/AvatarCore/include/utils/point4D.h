...
class Point4d {
   public:
    ...
    Point4d<T> operator+(T v) const {
        return Point4d<T>(x + v, y + v, z + v, w + v);
    }
    Point4d<T> operator-(T v) const {
        return Point4d<T>(x - v, y - v, z - v, w - v);
    }
    ...
    double getDist(const Point4d<T> &p) const { return sqrt(getDistSQ(p)); }
    ...
    Point3d<T> XYZ() const { return Point3d<T>(x, y, z); }
